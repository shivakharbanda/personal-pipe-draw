...
import { Upload, Cpu, AlertTriangle, FileCheck, ArrowRight, Loader2 } from 'lucide-react';
...
const StepIndicator: React.FC<StepIndicatorProps> = ({ currentStep, stepStates }) => {
  const getStepState = (stepId: WorkflowStep) => {
    if (!stepStates) return null;

    switch(stepId) {
      case WorkflowStep.RECOGNITION:
        return stepStates.recognition;
      case WorkflowStep.IDENTIFY_ERRORS:
        return stepStates.errorDetection;
      case WorkflowStep.GENERATE_UPDATED:
        return stepStates.generation;
      default:
        return null;
    }
  };
        ...
        return (
          <div key={step.id} className="relative">
            <div className={`
              ...
              ${isActive && isLoading ? 'bg-blue-50 border-blue-600 ring-2 ring-blue-600/20 animate-pulse' :
            ...
            `}>
              ...
              <p className="text-xs text-neutral-500 mt-1">
                {isLoading ? 'In Progress...' : step.description}
              </p>
            </div>

            {idx < steps.length - 1 && (
              <div className={`hidden md:block absolute -right-2 top-1/2 -translate-y-1/2 z-10 ${
                isCompleted ? 'text-teal-600' : 'text-neutral-400'
              }`}>
